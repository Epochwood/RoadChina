        ...
        switch (state.getValue(BLOCK_TYPE))
        {
            case FULL_BLOCK:
                switch (state.getValue(FACING)) {
                    ...
                    case SOUTH:
                    default:
                        return new AxisAlignedBB(-0.25D, 0.0D, -1.0D, 1.25D, 0.0D, 2.0D);
                    case EAST:
                    case WEST:
                        return new AxisAlignedBB(-1.0D, 0.0D, -0.25D, 2.0D, 0.0D, 1.25D);
                }
            case SLAB_BLOCK:
                switch (state.getValue(FACING)) {
                    case NORTH:
                    case SOUTH:
                    default:
                        return new AxisAlignedBB(-0.25D, -0.5D, -1.0D, 1.25D, -0.5D, 2.0D);
                    case EAST:
                    case WEST:
                        return new AxisAlignedBB(-1.0D, -0.5D, -0.25D, 2.0D, -0.5D, 1.25D);
                }
        }
        return null;
    }
